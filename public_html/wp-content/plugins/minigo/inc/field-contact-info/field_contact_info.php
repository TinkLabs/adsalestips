        ...
        public function render() {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                foreach ($contact_infos as $contact_info) {
                    ...
                    echo '<div class="redux-contact_info-accordion-group"><fieldset class="redux-field redux-container-select" data-id="'.$this->field['id'].'"><h3><span class="redux-contact_info-header">' . $contact_info['title'] . '</span></h3><div>';
                    ...
                    echo '<li><label for="' . $this->field['id'] . '-force_row_' . $x . '"><input type="checkbox" id="' . $this->field['id'] . '-force_row_' . $x . '" name="' . $this->field['name'] . '[' . $x . '][force_row]" value="1" class="checkbox contact_info-force_row" '.($contact_info['force_row'] == 1 ? 'checked="checked"' : '').' /> Force new row</label></li>';
            ...
            if ($x == 0) {
                echo '<div class="redux-contact_info-accordion-group"><fieldset class="redux-field redux-container-select" data-id="'.$this->field['id'].'"><h3><span class="redux-contact_info-header">New Slide</span></h3><div>';


                echo '<ul id="' . $this->field['id'] . '-ul" class="redux-contact_info-list">';
                $placeholder = (isset($this->field['placeholder']['title'])) ? esc_attr($this->field['placeholder']['title']) : __( 'Contact Info', 'redux-framework' );
                echo '<li><label>Text</label> <input type="text" id="' . $this->field['id'] . '-title_' . $x . '" name="' . $this->field['name'] . '[' . $x . '][title]" value="" placeholder="'.$placeholder.'" class="full-text contact_info-title" /></li>';
                $placeholder = (isset($this->field['placeholder']['url'])) ? esc_attr($this->field['placeholder']['url']) : __( 'Link (optional)', 'redux-framework' );
                echo '<li><label>URL</label> <input type="text" id="' . $this->field['id'] . '-url_' . $x . '" name="' . $this->field['name'] . '[' . $x . '][url]" value="" class="full-text" placeholder="'.$placeholder.'" /></li>';
                echo '<li><input type="hidden" class="contact_info-sort" name="' . $this->field['name'] . '[' . $x . '][sort]" id="' . $this->field['id'] . '-sort_' . $x . '" value="' . $x . '" /></li>';
                    ...
                    if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
                        ...
                        echo '<label>Icon</label> <select id="'.$this->field['id'].'-select" data-placeholder="'.$placeholder.'" name="' . $this->field['name'] . '[' . $x . '][select]" class="font-awesome-icons redux-select-item '.$this->field['class'].'" rows="6" style="width:93%;">';
                            echo '<option></option>';
                            foreach($this->field['options'] as $k => $v){
                                if (is_array($this->value)) {
